...
const App: React.FC = () => {
  ...
  const [settings, setSettings] = useState<UserSettings>(() => {
    const saved = localStorage.getItem('wendler_settings_v5');
  ...
  useEffect(() => {
    localStorage.setItem('wendler_settings_v5', JSON.stringify(settings));
  }, [settings]);
      ...
      <div className="space-y-6 pb-48 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-rose-50 rounded-[40px] p-8 border border-rose-100 shadow-sm">
           ...
           <button 
             ...
             className="w-full py-5 bg-white text-rose-600 font-black text-[11px] uppercase rounded-2xl shadow-xl shadow-rose-100 border border-rose-100 active:scale-95 transition-all"
           >
             프로그램 리셋하기
           </button>
        </div>
